...
def run_williams(state):
    ...
    primal_file = "Primal_Lorenzo/" + state + "_primal.txt"
    dual_file = "Dual_Lorenzo/" + state + "_dual.txt"
    population_file = "Population/" + state + "_population.txt"
    face_txt = "Faces/" + state + "_faces.txt"
    return Williams_test.Williams_test(primal_file, dual_file, population_file, face_txt, num_district, state)

###########################
# Run An Instance using Hess' model
###########################
def run_Hess(state):
    population = read.read_population("Population/"+state+"_population.txt", "Hess")
    total_pop = population[-1]
    L = (total_pop / 2) * (0.0995)
    U = (total_pop / 2) * (1.005)
    population.pop(-1)
    num_district = states_rows[state][3]
    return Hess.Hess(state, L, U, num_district, population)
