...
if is_forest == True:
    ...
    x = m.addVars(primal_graph.edges, vtype = GRB.BINARY, name="x")
    # Create root variables
    r = m.addVars(primal_nodes, name="r")
    # Constraint 1
    m.addConstrs(gp.quicksum(x[out_edge] for out_edge in primal_graph.out_edges(i))  <= gp.quicksum(w[out_edge]
                                            for out_edge in primal_graph.out_edges(i)) for i in range(len(primal_file)))
    # Constraint 2
    m.addConstr(gp.quicksum(r[node] for node in primal_nodes) == k)
    # Constraint 3
    m.addConstrs(r[node] + gp.quicksum(x[predecessor, node] for predecessor in primal_graph.predecessors(node)) == 1
...
if is_population_considered and is_forest:
    ...
    population.add_population_constraints(m, r, x, p, primal_nodes, primal_graph, primal_edges, add_objective, d_dict)
